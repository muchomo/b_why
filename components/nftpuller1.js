...
import { nftContract1, key, displayAmount, mainnet } from './settings';
import { oabi } from '../constants/oabi';


export default function NftPuller1() {
  ...
  useEffect(() => {
    ...
    async function generateNft() {
      ...
      const contract = new ethers.Contract(nftContract1, oabi, wallet);
      const itemArray = [];
...
if (loadingState === 'loaded' && !nfts.length)
    ...
    return (
      <Container md>
        <Text h1 css={{marginLeft:'$10',color: 'white'}} size={'40px'}>Superbowl OT Squares</Text>
